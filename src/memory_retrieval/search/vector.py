...
class _VectorSearchBase(SearchBackendBase):
    """Shared sqlite-vec DB logic for all vector-search backends.

    Subclasses must set `embedding_model` and `vector_dimensions` attributes
    and implement `_get_embedding` and `_get_embeddings_batch`.
    """
    ...
    def insert_memories(self, db_path: str, memories: list[dict[str, Any]]) -> int:
        ...
        print(f"  Generating {len(situations)} embeddings (batch)...")
    ...
    def rebuild_database(self, db_path: str, memories_dir: str) -> None:
        ...
        print(
            f"Inserting memories (embedding model={self.embedding_model}, dim={self.vector_dimensions})..."
        )
...
class VectorBackend(_VectorSearchBase):
